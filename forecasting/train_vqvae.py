...
logger.setLevel(logging.INFO) # Change this to INFO, WARNING, ERROR, or CRITICAL to adjust verbosity
...
def main(device, config, save_dir, logger, data_init_loc, args):
    ...
    logger.info(f"Logging parameters with the Python in-built logger: {config}")
    ...
    for epoch in range(int((vqvae_config['num_training_updates']/len(train_loader)) + 0.5)):
        ...
        for i, (batch_x) in enumerate(train_loader):
            ...
            loss, vq_loss, recon_error, x_recon, perplexity, embedding_weight, encoding_indices, encodings = \
                model.shared_eval(tensor_all_data_in_batch, optimizer, 'train', logger)
        ...
        if epoch % 10000 == 0:
            ...
            logger.info(f'Saved model from epoch : {epoch}')

    logger.info(f"total time: {round(time.time() - start_time, 3)} seconds")
...
def create_datloaders(batchsize=100, dataset="dummy", base_path='dummy'):
    ...
    elif dataset == 'base_odom':
        print("base_odom")
        full_path = base_path + '/base_odom'
...
if __name__ == '__main__':
    ...
    logger.info('Config folder:\t {}'.format(config_file))


    # Load JSON config file
    with open(config_file, 'r') as f:
        config = json.load(f)
    logger.info(f'Running Config: {config_file}')
    ...
    print("FINAL_MODEL_PATH: " + save_dir + '/checkpoints/final_model.pth')
    ...
    # else:
    #     print('DEBUG:Comet logging is disabled.')
    #       comet_logger = None
    ...
    # call main
    main(device, config, save_dir, logger, data_init_loc, args)
