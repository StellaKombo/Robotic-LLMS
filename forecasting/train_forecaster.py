...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_dataloader is not None:
            ...
            with torch.no_grad():
                for i, vdata in enumerate(val_dataloader):
                    ...
                    if i == 0:
                        all_nominal_measurements = [labels_time.detach().cpu().numpy()]
                    else:
                        all_nominal_measurements.append(labels_time.detach().cpu().numpy())
            ...
            np.save(
                os.path.join(args.file_save_path, f"Prediction_val_Tin{args.Tin}_Tout{args.Tout}_seed{args.seed}.npy"),
                np.concatenate(all_prediction_time, axis=0)
            )

            np.save(
                os.path.join(args.file_save_path, f"Nominal_val_Tin{args.Tin}_Tout{args.Tout}_seed{args.seed}.npy"),
                np.concatenate(all_nominal_measurements, axis=0)
            )

            print(
                f"Values for validation prediction and the actual measurements are saved to {args.file_save_path}!"
            )
        ...
        if test_dataloader is not None:
            ...
            np.save(
                os.path.join(args.file_save_path, f"Prediction_test_Tin{args.Tin}_Tout{args.Tout}_seed{args.seed}.npy"),
                np.concatenate(all_prediction_time, axis=0)
            )

            np.save(
                os.path.join(args.file_save_path, f"Nominal_test_Tin{args.Tin}_Tout{args.Tout}_seed{args.seed}.npy"),
                np.concatenate(all_nominal_measurements, axis=0)
            )

            print(
                f"Values for test prediction and the actual measurements are saved to {args.file_save_path}!"
            )
